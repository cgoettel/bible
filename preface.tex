...
\textsc{alt} = alternatively

\textsc{ed} = Editorial (author's) note
...
\textsc{halot} = Koehler and Baumgartner's \textit{Hebrew and Aramaic Lexicon of the Old Testament}

Davidson = \textit{The Analytical Hebrew and Chaldee Lexicon} by Benjamin Davidson

\textdegree\dots$\mathscr{U}$ symbolizes that the English word is understood from the Hebrew, but not explicitly said.

\textsc{bhs} = \textit{Biblia Hebraica Stuttgartensia}

W\&O = Waltke and O'Connor's \textit{An Introduction to Biblical Hebrew Syntax}
