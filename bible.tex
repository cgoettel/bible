...
\documentclass[twoside,twocolumn]{book} % extbook allows for smaller font sizes.

% Page layout options
% \usepackage[paperwidth=4.6in, paperheight=6in, margin=1cm]{geometry} % Small size, like my personal scriptures.
\usepackage[paperwidth=5.75in, paperheight=8in, top=1in, bottom=1in, left=0.5in, right=0.5in, headsep=0.5in]{geometry} % This is an abomination. The page height needs to be a bit longer, but it can't be because there are too many footnotes. Fix that bug, then fix this.
...
% Defining two new commands which will be dynamically changed on each call of \chap
...
\begin{document}
    \frontmatter
    \pagestyle{empty}
    \titleformat{\chapter}[display]{\centering\Huge}{}{0ex}{#1}
    ...
    \titleformat{\section}{\centering}{}{1em}{\englishchaptertitle\ #1}
